...
*/
    ...
    /**
* Property: baseUrl
* {String} baseUrl for the current KML. Help to resolve
* the network links or urls inside ths current KML
*/
    baseUrl : "",
...
*/
    parseLinks: function(nodes, options) {
        ...
        for(var i=0, len=nodes.length; i<len; i++) {
            ...
            // resolve relative url (if any) to right path
            href = this.resolvePath(href);
            if(href && !this.fetched[href]) {
                this.fetched[href] = true; // prevent reloading the same urls
                var data = this.fetchLink(href);
                if (data) {
                    // remember the href for inner this.resolvePath
                    this.baseUrl = href;
...
*/
    parseFeature: function(node) {
        ...
        var order = ["MultiGeometry", "Polygon", "LineString", "Point", "LatLonBox"];
...
*/
        ...
        /**
* Method: parseGeometry.latlonbox
* Given a KML node representing a LatLonBox geometry, create an OpenLayers
* polygon geometry.
*
* Parameters:
* node - {DOMElement} A KML LatLonBox node.
*
* Returns:
* {<OpenLayers.Geometry.Point>} A polygon geometry.
*/
        latlonbox : function(node) {
            // parse bounds
            var children = node.childNodes;
            var child, north, south, east, west;
            for(var i=0, len=children.length; i<len; ++i ) {
                child = children[i];
                if(child.nodeType == 1) {
                    var name = (child.prefix) ? child.nodeName.split(":")[1] : child.nodeName;
                    if (name == 'north') {
                      north = parseFloat(child.firstChild.nodeValue);
                    } else if (name == 'south') {
                      south = parseFloat(child.firstChild.nodeValue);
                    } else if (name == 'east') {
                      east = parseFloat(child.firstChild.nodeValue);
                    } else if (name == 'west') {
                      west = parseFloat(child.firstChild.nodeValue);
                    }
                }
            }
            
            // convert to polygon geometry
            var left = Math.min(east, west);
            var bottom = Math.min(north, south);
            var right = Math.max(east, west);
            var top = Math.max(north, south);
            var bounds = new OpenLayers.Bounds(left, bottom, right, top);
            var geometry = bounds.toGeometry();
            
            // add a special field to indicate it is ground overlay
            // find the Icon.href
            var siblings = node.parentNode.childNodes;
            for(var i=0, ii=siblings.length; i<ii; ++i ) {
                var sibling = siblings[i];
                var siblingName = (sibling.prefix) ? sibling.nodeName.split(":")[1] : sibling.nodeName;
                if(siblingName == 'Icon') {
                    var iconChildren = sibling.childNodes;
                    for(var j=0, jj=iconChildren.length; j<jj; ++j ) {
                        var child = iconChildren[j];
                        if(child.nodeType == 1) {
                            var name = (child.prefix) ? child.nodeName.split(":")[1] : child.nodeName;
                            if (name == 'href') {
                                var href = child.firstChild.nodeValue;
                                if (href) {
                                    href = this.resolvePath(href);
                                    geometry.groundOverlayHref = href;
                                }
                            }
                        }
                    }
...
*/
    ...
    resolvePath: function(href) {
        // if absolute path
        if (!this.baseUrl || /^(ftp:\/\/|http:\/\/|https:\/\/|\/)/.test(href)) {
          return href;
        }
        
        // the this.baseUrl is not useful
        var lastSlash = this.baseUrl.lastIndexOf('/');
        if (lastSlash <= 0) {
          return href;
        }
        
        return this.baseUrl.substring(0, lastSlash)+'/'+href;
    },
