...
*/
    setStyle: function(node, style, options, geometry) {
        ...
        // Fix the style for ground overlay
        if (node._geometryClass === "OpenLayers.Geometry.Polygon" && geometry.groundOverlayHref) {
            node.fillcolor = "none";
            node.stroked = false;
            ...
            var fills = node.getElementsByTagName("fill");
            var fill = (fills.length == 0) ? null : fills[0];
            fill.opacity = style.graphicOpacity || 1.0;
...
*/
    drawPolygon: function(node, geometry) {
        // drawing for ground overlay
        if (geometry.groundOverlayHref) {
            var resolution = this.getResolution();
            var bound = geometry.getBounds();
        
            node.style.left = ((bound.left/resolution - this.offset.x) | 0) + "px";
            node.style.top = ((bound.bottom/resolution - this.offset.y) | 0) + "px";
            node.style.width = ((bound.getWidth()/resolution) | 0) + "px";
            node.style.height = ((bound.getHeight()/resolution) | 0) + "px";
        
            // fill node for containing the image
            var fills = node.getElementsByTagName("fill");
            var fill = (fills.length == 0) ? null : fills[0];
            if (!fill) {
                fill = this.createNode('olv:fill', node.id + "_fill");
            ...
            fill.on = "true";
            fill.type = "frame";
            fill.src = geometry.groundOverlayHref;
            if (fill.parentNode != node) {
                node.appendChild(fill);
            }
